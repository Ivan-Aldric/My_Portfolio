  ...
  return (
    ...
    >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
        ...
        >
          <motion.h2
            className="text-3xl sm:text-4xl md:text-5xl font-bold bg-gradient-to-r from-slate-900 via-blue-800 to-slate-900 dark:from-white dark:via-blue-200 dark:to-white bg-clip-text text-transparent mb-6"
        ...
        <div className="grid lg:grid-cols-3 gap-8 lg:gap-12">
          ...
          >
            <div className="space-y-4 lg:space-y-6">
              <h3 className="text-2xl sm:text-3xl font-bold text-slate-900 dark:text-white flex items-center gap-3">
                <div className="w-1 h-6 sm:h-8 bg-gradient-to-b from-blue-500 to-violet-500 rounded-full" />
                Professional Bio
              </h3>
              <div className="space-y-4 lg:space-y-6 text-slate-600 dark:text-slate-300 leading-relaxed text-base sm:text-lg">
                <motion.p
                  initial={{ opacity: 0, y: 20 }}
                  whileInView={{ opacity: 1, y: 0 }}
            ...
            <div className="space-y-6 lg:space-y-8">
              <h3 className="text-2xl sm:text-3xl font-bold text-slate-900 dark:text-white flex items-center gap-3">
                <div className="w-1 h-6 sm:h-8 bg-gradient-to-b from-emerald-500 to-blue-500 rounded-full" />
                Core Values
              </h3>
              <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 lg:gap-6">
                {coreValues.map((value, index) => (
                  <motion.div
                    ...
                    className={`${value.bgColor} ${value.hoverColor} p-6 lg:p-8 rounded-2xl border border-white/20 dark:border-slate-700/50 backdrop-blur-sm transition-all duration-300 cursor-pointer group shadow-lg hover:shadow-xl`}
                  >
                    <motion.div
                      className={`w-12 h-12 lg:w-14 lg:h-14 bg-gradient-to-br ${value.color} rounded-xl flex items-center justify-center mb-4 lg:mb-6 group-hover:scale-110 transition-transform duration-300`}
                      animate={{ rotate: hoveredValue === index ? 360 : 0 }}
                      transition={{ duration: 0.6 }}
                    >
                      <value.icon className="text-white text-lg lg:text-xl" />
                    </motion.div>
                    <h4 className="font-bold text-slate-900 dark:text-white mb-2 lg:mb-3 text-base lg:text-lg">{value.title}</h4>
                    <p className="text-slate-600 dark:text-slate-300 leading-relaxed text-sm lg:text-base">{value.description}</p>
                  </motion.div>
                ))}
              </div>
            </div>
            ...
            <div className="space-y-6 lg:space-y-8">
              <h3 className="text-2xl sm:text-3xl font-bold text-slate-900 dark:text-white flex items-center gap-3">
                <div className="w-1 h-6 sm:h-8 bg-gradient-to-b from-violet-500 to-pink-500 rounded-full" />
                Soft Skills
              </h3>
              <motion.div
              ...
              >
                {softSkills.map((skill, index) => (
                  <motion.span
                    ...
                    className={`${skill.color} px-3 sm:px-4 lg:px-6 py-2 sm:py-3 rounded-full font-medium border-2 backdrop-blur-sm hover:shadow-lg transition-all duration-300 cursor-default text-xs sm:text-sm`}
                  >
                    {skill.name}
                  </motion.span>
                ))}
          ...
          >
            <motion.div
              className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm rounded-2xl p-6 lg:p-8 text-center border border-white/20 dark:border-slate-700/50 shadow-xl"
              whileHover={{ y: -5 }}
              transition={{ duration: 0.3 }}
            >
              <motion.div
                className="w-24 h-24 sm:w-32 sm:h-32 lg:w-36 lg:h-36 rounded-full mx-auto mb-4 lg:mb-6 overflow-hidden shadow-2xl border-4 border-white dark:border-slate-700"
              ...
              <h4 className="text-lg sm:text-xl font-bold text-slate-900 dark:text-white mb-2">LEKEUFACK IVAN ALDRIC</h4>
              <p className="text-slate-600 dark:text-slate-300 mb-4 lg:mb-6 font-medium text-sm sm:text-base">MIS Specialist & IT Consultant</p>
              <div className="space-y-2 lg:space-y-3 text-slate-600 dark:text-slate-300">
                <div className="flex items-center justify-center gap-2 lg:gap-3 hover:text-blue-600 dark:hover:text-blue-400 transition-colors text-sm sm:text-base">
            ...
            <motion.div
              className="bg-gradient-to-br from-blue-50/80 to-violet-50/80 dark:from-blue-900/20 dark:to-violet-900/20 backdrop-blur-sm rounded-2xl p-6 lg:p-8 border border-white/20 dark:border-slate-700/50 shadow-xl"
              whileHover={{ y: -5 }}
              transition={{ duration: 0.3 }}
            >
              <h4 className="font-bold text-slate-900 dark:text-white mb-4 lg:mb-6 text-base lg:text-lg">Hobbies & Passion</h4>
              <div className="space-y-3 lg:space-y-4">
                {hobbies.map((hobby, index) => (
                  <motion.div
                    ...
                    transition={{ duration: 0.4, delay: index * 0.1 }}
                    whileHover={{ x: 5 }}
                    className="flex items-center gap-2 lg:gap-3 text-slate-600 dark:text-slate-300 hover:text-blue-600 dark:hover:text-blue-400 transition-all duration-300 cursor-default"
                  >
                    <div className="w-6 h-6 lg:w-8 lg:h-8 bg-gradient-to-br from-blue-500 to-violet-500 rounded-lg flex items-center justify-center flex-shrink-0">
                      <hobby.icon className="w-3 h-3 lg:w-4 lg:h-4 text-white" />
                    </div>
                    <span className="font-medium text-sm lg:text-base">{hobby.text}</span>
                  </motion.div>
                ))}
              </div>
            </motion.div>
